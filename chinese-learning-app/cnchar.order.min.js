!function(j, f) {
    "object" == typeof exports && "object" == typeof module ? module.exports = f() : "function" == typeof define && define.amd ? define([], f) : "object" == typeof exports ? exports.cncharOrder = f() : j.cncharOrder = f()
}(this, (function() {
    return function(j) {
        var f = {};
        function s(k) {
            ...
            return j[k].call(c.exports, c, c.exports, s),
            c.l = !0,
            c.exports
        }
        return s.m = j,
        s.c = f,
        s.d = function(j, f, k) {
            s.o(j, f) || Object.defineProperty(j, f, {
                enumerable: !0,
                get: k
            })
        }
    ...
    , function(j, f, s) {
        ...
        var n = function(j) {
            for (var f = [], s = 1; s < arguments.length; s++)
                f[s - 1] = arguments[s];
            "string" == typeof j && (j = j.split(" ")),
            r.checkArgs("orderToWord", f);
            var k = []
              , c = "";
            if (j.forEach((function(j) {
                n.orders[j] ? c += n.orders[j].letter : k.push(j)
            }
            )),
            k.length > 0)
                return console.error("orderToWord: 参数笔画名数组有误：" + k.join(",")),
                [];
            var e = []
              , i = {
                start: r.has(f, d.start),
                match: r.has(f, d.match),
                matchorder: r.has(f, d.matchorder),
                contain: r.has(f, d.contain),
                simple: r.has(f, d.simple),
                trad: r.has(f, d.trad),
                array: r.has(f, d.array)
            };
            return i.simple || i.trad || (i.simple = i.trad = !0),
            i.simple && g(e, c, i, l),
            i.trad && r.dict.getTradOrders && g(e, c, i, r.dict.getTradOrders()),
            r.has(f, d.array) ? e : e.join("")
        }
          , g = function(j, f, s, k) {
            if (s.match)
                for (var c in k) {
                    for (var l = !1, e = 0; e < f.length; e++)
                        if (-1 === k[c].indexOf(f[e])) {
                            l = !0;
                            break
                        }
                    l || j.push(c)
                }
            else if (s.matchorder)
                for (var c in k) {
                    l = !1;
                    var i = k[c];
                ...
                for (var c in k)
                    -1 !== k[c].indexOf(f) && j.push(c);
            else if (s.start)
                for (var c in k)
                    0 === k[c].indexOf(f) && j.push(c);
            else
                for (var c in k)
                    k[c] === f && j.push(c);
            return j
        ...
        function u(j, f, s, k) {
            ...
            k || delete c.sameLetterTo,
            n.orders[j] = c
        }
        !function() {
            for (var j in n.orders = {},
            n._base = g,
            e) {
                var f = e[j]
                  , s = f.name
        ...
        var h, b = function() {
            for (var j = 0, f = 0, s = arguments.length; f < s; f++)
                j += arguments[f].length;
            var k = Array(j)
              , c = 0;
            for (f = 0; f < s; f++)
                for (var l = arguments[f], e = 0, i = l.length; e < i; e++,
                c++)
                    k[c] = l[e];
            return k
        }, t = {
        ...
        function a(j, f, s, k) {
            ...
            for (var c = 0; c < j.length; c++)
                -1 === k.indexOf(c) && "string" == typeof j[c] && (j[c] = m(f[c], j[c], s));
            return j
        }
        function m(j, f, s) {
            if (void 0 === f)
                return j;
            var k = h.has(s, t.detail)
              , c = t.letter;
            if (k ? c = t.detail : h.has(s, t.shape) ? c = t.shape : h.has(s, t.name) ? c = t.name : h.has(s, t.count) && (c = t.count),
            c === t.count)
                return f.length;
            if (c === t.letter)
                return f;
        ...
        var p = {
            ...
            orderToWord: n,
            install: function(j) {
                var f = j._origin.stroke;
                h = j._,
                j.setOrder = o;
                var s = function(j) {
                    for (var s = [], k = 1; k < arguments.length; k++)
                        s[k - 1] = arguments[k];
                    return h.has(s, t.order) ? y.apply(void 0, b([j], s)) : f.apply(void 0, b([j], s))
                };
                j.stroke = s,
                String.prototype.stroke = function() {
                    for (var j = [], f = 0; f < arguments.length; f++)
                        j[f] = arguments[f];
                    return s.apply(void 0, b([this], j))
                }
                ,
                j._.order = !0,
                j.type.stroke = t,
                j._.orderWithLetters = a,
                j._._reinitStrokeOrder && (j._._reinitStrokeOrder(),
                delete j._._reinitStrokeOrder),
                function(j) {
                    j.orderToWord = n,
                    j.type.orderToWord = d,
                    r = j._
                }(j)
            },
            dict: {
        ...
        "object" == typeof window && window.CnChar && (window.CncharOrder = p,
        window.CnChar.use(p));
        f.default = p
    }
    ]).default
